...
class DeletePopup(QtWidgets.QFrame):
    def __init__(self, font, delete_from_playlist_t, delete_from_drive_t, del_path):
        super().__init__()

        self.font = font
        self.delete_from_playlist_t = delete_from_playlist_t
        self.delete_from_drive_t = delete_from_drive_t
        ...
        for text in [
            self.delete_from_playlist_t,
            self.delete_from_drive_t
        ]:
            btn = QtWidgets.QPushButton(text)
            btn.setFont(self.font)
            btn.setCursor(QtGui.QCursor(QtCore.Qt.PointingHandCursor))
            btn.clicked.connect(lambda checked, t=text: self.funcs(t))
            inner.addWidget(btn)

        layout.addWidget(container)

    def funcs(self, t):
        if t == self.delete_from_playlist_t:
            print("del_playlist")
            window.delete_from_playlist(self.del_path)
        elif t == self.delete_from_drive_t:
            print("del_drive")
            window.delete_current_track(self.del_path)
...
class TrackFr(QtWidgets.QFrame):
...
class MacButton(QtWidgets.QToolButton):
    def __init__(self, color, icon_path=None, parent=None):
        super().__init__(parent)
        ...
        self.setIconSize(QtCore.QSize(9, 9))
...
class TitleBar(QtWidgets.QWidget):
    def __init__(self, parent):
        ...
        self.close_button = MacButton(
            "#ff5f57",
            resource_path("assets/icons/close-black.png")
        )
        self.close_button.clicked.connect(self.parent_window.close)

        self.minimize_button = MacButton(
            "#febc2e",
            resource_path("assets/icons/minimize-black.png")
        )
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        if build_os == "Darwin":
            ...
            self.colors_file = f"/Users/{username}/Afory/colors.json"
        else:
            ...
            self.colors_file = f"C:/Users/{username}/Afory/colors.json"
        ...
        self.main_font.setPointSize(int(self.settings.get("font_size", 12)))
        ...
        self.delete_from_playlist_t = "Delete from playlist"
        self.delete_from_drive_t = "Delete from drive"
        self.warning_lost_tracks = "Warning: Lost tracks detected."
        self.lost_tracks_t = "Lost tracks:"
        self.lost_tracks_exp = "It means that path to these tracks does not exist."
        self.manually_adding = "You can add lost tracks manually by using Add mp3 button."
        self.application_font = "Application font:"
        self.web_download_t = "Download..."
    ...
    def setupUi(self):
        bg_color = self.colors.get("bg_color")
        self.setStyleSheet(
                "QMainWindow {"
        ...
        layout2 = QtWidgets.QVBoxLayout(container)
        layout2.setContentsMargins(0, 0, 0, 0)
        layout2.setSpacing(0)

        self.title_bar = TitleBar(self)
        layout2.addWidget(self.title_bar)
        ...
        shadow_s = QtWidgets.QGraphicsDropShadowEffect()
        shadow_s.setBlurRadius(30)
        shadow_s.setXOffset(5)
        shadow_s.setYOffset(5)
        shadow_s.setColor(QtGui.QColor(0, 0, 0, 90))

        shadow_l = QtWidgets.QGraphicsDropShadowEffect()
        shadow_l.setBlurRadius(30)
        shadow_l.setXOffset(5)
        shadow_l.setYOffset(5)
        shadow_l.setColor(QtGui.QColor(0, 0, 0, 90))

        island_color = self.colors.get("islands_color")

        version_label = QtWidgets.QLabel(self)
        version_label.setStyleSheet("color: white; background-color: transparent")
        version_label.setText(f"App version: {script_version}")
        version_label.setGeometry(QtCore.QRect(70, 8, 150, 15))
        version_label.setFont(self.main_font)

        self.sidebar = QtWidgets.QFrame(self)
        self.sidebar.setGeometry(QtCore.QRect(10, 25, 201, 540))
        ...
        self.sidebar.setGraphicsEffect(shadow_s)
        self.sidebar.setStyleSheet(f"background-color: {island_color};\n"
                            "border-color: transparent;\n"
                            "border-radius: 10px;")

        self.leftbar = QtWidgets.QFrame(self)
        self.leftbar.setGeometry(QtCore.QRect(221, 25, 593, 540))
        ...
        self.leftbar.setGraphicsEffect(shadow_l)
        self.leftbar.setStyleSheet(f"""
            background-color: {island_color};
            border-radius: 10px;
        """)

        self.add_playlist_btn = QtWidgets.QPushButton(self.sidebar)
        self.add_playlist_btn.setGeometry(QtCore.QRect(10, 60, 181, 31))
        ...
        self.afory_label.setGeometry(QtCore.QRect(25, 15, 150, 31))
        ...
        self.delete_playlist_btn.setGeometry(QtCore.QRect(10, 100, 181, 31))
        ...
        self.playlists_frame.setGeometry(QtCore.QRect(10, 140, 181, 335))
        ...
        self.settings_btn.setGeometry(QtCore.QRect(10, 485, 181, 31))
        ...
        self.Tracks_frame.setStyleSheet("""
            background-color: transparent;
            border-radius: 5px;""")
        self.Tracks_frame.setGeometry(QtCore.QRect(230, 85, 571, 280))
        ...
        self.label_current_playlist.setGeometry(QtCore.QRect(230, 40, 571, 31))
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(20)
        font.setBold(True)
        ...
        shadow_t = QtWidgets.QGraphicsDropShadowEffect()
        shadow_t.setBlurRadius(30)
        shadow_t.setXOffset(5)
        shadow_t.setYOffset(5)
        shadow_t.setColor(QtGui.QColor(0, 0, 0, 90))

        self.toolbar = QtWidgets.QFrame(self)
        self.toolbar.setGeometry(QtCore.QRect(247, 375, 536, 31))
        ...
        self.add_mp3_btn = QtWidgets.QPushButton(self.toolbar)
        self.add_mp3_btn.setGeometry(QtCore.QRect(0, 0, 131, 31))
        self.add_mp3_btn.setCursor(QtGui.QCursor(QtCore.Qt.PointingHandCursor))
        self.add_mp3_btn.setObjectName("add_mp3_btn")
        self.add_mp3_btn.clicked.connect(lambda: self.add_local_file())
        ...
        self.web_download_btn = QtWidgets.QPushButton(self.toolbar)
        self.web_download_btn.setGeometry(QtCore.QRect(135, 0, 131, 31))
        self.web_download_btn.setCursor(QtGui.QCursor(QtCore.Qt.PointingHandCursor))
        self.web_download_btn.clicked.connect(lambda: self.download_youtube())
        self.web_download_btn.setFont(self.main_font)

        self.shuffle_btn = QtWidgets.QPushButton(self.toolbar)
        self.shuffle_btn.setGeometry(QtCore.QRect(270, 0, 131, 31))
        ...
        self.repeat_btn = QtWidgets.QPushButton(self.toolbar)
        self.repeat_btn.setGeometry(QtCore.QRect(405, 0, 131, 31))
        ...
        self.current_track_label.setGeometry(QtCore.QRect(230, 410, 571, 20))
        ...
        self.play_pause_btn.setGeometry(QtCore.QRect(480, 435, 71, 71))
        ...
        self.prev_track_btn.setGeometry(QtCore.QRect(420, 445, 51, 51))
        ...
        self.next_track_btn.setGeometry(QtCore.QRect(560, 445, 51, 51))
        ...
        self.volume_lab.setGeometry(620, 458, 25, 25)

        self.volume_slider = QtWidgets.QSlider(QtCore.Qt.Horizontal, self)
        self.volume_slider.setGeometry(650, 450, 60, 40)
        ...
        self.seekbar.setGeometry(250, 515, 535, 5)
        ...
        self.current_track_label_2.setGeometry(QtCore.QRect(230, 535, 571, 20))
        ...
        '''self.version_label_mw = QtWidgets.QLabel(self)
        self.version_label_mw.setGeometry(QtCore.QRect(13, 550, 500, 30))
        self.version_label_mw.setText(f"Version: {script_version}. Developed by Tuseloryy.")
        self.version_label_mw.setStyleSheet("color: white;")
        self.version_label_mw.setFont(self.main_font)'''
    ...
    def setupText(self):
        ...
        self.add_mp3_btn.setText(self.add_mp3)
        self.web_download_btn.setText(self.web_download_t)
        self.shuffle_btn.setText(self.shuffle_text)
    ...
    def setupTooltip(self):
        ...
        self.web_download_btn.setToolTip(self.download_from_YT)
        self.shuffle_btn.setToolTip(self.shuffle_tooltip)
        self.repeat_btn.setToolTip(self.repeat_tooltip)
        self.play_pause_btn.setToolTip(self.play_pause_tooltip)
        self.prev_track_btn.setToolTip(self.prev_track_tooltip)
    ...
    def setupIcons(self):
        ...
        self.add_playlist_btn.setIconSize(QtCore.QSize(23, 23))
        self.delete_playlist_btn.setIcon(QtGui.QIcon(resource_path("assets/icons/bin-color.png")))
        self.add_mp3_btn.setIcon(QtGui.QIcon(resource_path("assets/icons/plus-symbol.png")))
        self.add_mp3_btn.setIconSize(QtCore.QSize(23, 23))
        self.web_download_btn.setIcon(QtGui.QIcon(resource_path("assets/icons/download.png")))
        self.web_download_btn.setIconSize(QtCore.QSize(23, 23))
    ...
    def load_save_folder(self):
        if build_os == "Darwin":
            if os.path.exists(f"/Users/{username}/Afory/SAVEFOLDER.txt"):
                with open(f"/Users/{username}/Afory/SAVEFOLDER.txt", "r", encoding="utf-8") as f:
                    return f.read()
            return f"/Users/{username}/Afory/downloads"
        else:
            if os.path.exists(f"C:/Users/{username}/Afory/SAVEFOLDER.txt"):
                with open(f"C:/Users/{username}/Afory/SAVEFOLDER.txt", "r", encoding="utf-8") as f:
                    return f.read()
            return f"C:/Users/{username}/Afory/downloads"

    def save_save_folder(self):
        if build_os == "Darwin":
            with open(f"/Users/{username}/Afory/SAVEFOLDER.txt", "w", encoding="utf-8") as f:
                f.write(self.save_folder)
        else:
            with open(f"C:/Users/{username}/Afory/SAVEFOLDER.txt", "w", encoding="utf-8") as f:
                f.write(self.save_folder)
    ...
    def load_settings(self):
        default_settings = {"volume": 0.7, "font": "Comic Sans MS", "font_size": 12, "language": "English", "change_volume": True}
    ...
    def save_colors(self):
        try:
            with open(self.colors_file, "w", encoding="utf-8") as f:
                json.dump(self.colors, f, ensure_ascii=False, indent=4)
        except Exception as e:
            print(f"Save error: {e}")

    def load_colors(self):
        default_colors = {"bg_color": "#1d1f28", "islands_color": "#292934"}
    ...
    def apply_lang(self):
        ...
        elif self.current_lang == "Português":
            self.load_language("pt")
        elif self.current_lang == "বাংলা":
            self.load_language("bn")
        self.setupText()
        self.setupTooltip()
        self.update_playlist_menu()
        self.refresh_tracks_display()
    ...
    def update_playlist_menu(self):
        color = self.colors.get("islands_color")
        ...
        for name in self.playlists.keys():
            ...
            pl_frame.setAcceptDrops(True)
            pl_frame.setStyleSheet(f"""
                QFrame {{
                    background-color: {"#3c4259" if is_active else color};
                    border-radius: 5px;
                }}
            """)
            ...
            def dropEvent(event, playlist_name=name):
                ...
                if source_path in self.playlists[playlist_name]:
                    return
                self.playlists[playlist_name].append(source_path)
            ...
            pl_frame.dragEnterEvent = dragEnterEvent
            pl_frame.dropEvent = dropEvent
            pl_frame.dragLeaveEvent = dragLeaveEvent
    ...
    def refresh_tracks_display(self):
        ...
        for index, path in enumerate(playlist):
            ...
            track_row.setMouseTracking(True)

            if not os.path.exists(path):
                lost_tracks.append(path)
            ...
            track_row.setFixedHeight(30)
            track_row.setStyleSheet(f"""QFrame {{background-color:{"#3c4259" if is_active else color};
                           border-radius: 5px;
                           }}""")
            ...
            if os.path.exists(path):
                ...
                def show_popup(path):
                    self.del_popup = DeletePopup(font=self.main_font,
                                                 delete_from_playlist_t=self.delete_from_playlist_t,
                                                 delete_from_drive_t=self.delete_from_drive_t, del_path=path)

                    pos = del_btn.mapToGlobal(
                        QtCore.QPoint(
                            del_btn.width(),
                            -self.del_popup.sizeHint().height()
                        )
                    )

                    self.del_popup.move(pos)
                    self.del_popup.show()

                move_btn = DragButton(path, self)
                move_btn.setIcon(QtGui.QIcon(resource_path("assets/icons/move.png")))
                move_btn.setIconSize(QtCore.QSize(15,15))
                move_btn.setCursor(QtGui.QCursor(QtCore.Qt.PointingHandCursor))
                move_btn.setFixedWidth(15)
                move_btn.setFixedHeight(15)
                move_btn.setObjectName("move_btn")
                move_btn.setToolTip(self.move_tooltip)

                row_layout.addWidget(move_btn)

                btn = QtWidgets.QPushButton(f"{os.path.basename(path)}")
                btn.setObjectName("btn")
                btn.setToolTip(self.select_track_tooltip)
                btn.setFixedHeight(30)
                btn.setFixedWidth(410)
                btn.setCursor(QtGui.QCursor(QtCore.Qt.PointingHandCursor))
                btn.setFont(self.main_font)
                btn.clicked.connect(
                ...
                del_btn.clicked.connect(
                    lambda checked, p=path: show_popup(p)
                )
                ...
                buttons_layout.addWidget(ren_btn)

                opacity = QtWidgets.QGraphicsOpacityEffect(buttons_widget)
                buttons_widget.setGraphicsEffect(opacity)

                opacity.setOpacity(0)
                row_layout.addWidget(buttons_widget)

                fade_in = QtCore.QPropertyAnimation(opacity, b"opacity")
                fade_in.setDuration(100)
                fade_in.setStartValue(0)
                fade_in.setEndValue(1)

                fade_out = QtCore.QPropertyAnimation(opacity, b"opacity")
                fade_out.setDuration(100)
                fade_out.setStartValue(1)
                fade_out.setEndValue(0)

                track_row.fade_in = fade_in
                track_row.fade_out = fade_out
                ...
                def enterEvent(event,
                    ...
                    fade_in.stop()
                    fade_in.setStartValue(widget.graphicsEffect().opacity())
                    fade_in.setEndValue(1)
                    fade_in.start()
                ...
                def leaveEvent(event,
                    ...
                    fade_out.stop()
                    fade_out.setStartValue(widget.graphicsEffect().opacity())
                    fade_out.setEndValue(0)
                    fade_out.start()

                track_row.enterEvent = enterEvent
                track_row.leaveEvent = leaveEvent
        ...
        if lost_tracks:
            ...
            dialog.setText(f"{self.warning_lost_tracks}\n"
                                                    f"{self.lost_tracks_t}  {lost_tracks}\n"
                                                    f"{self.lost_tracks_exp}\n"
                                                    f"{self.manually_adding}")
            del_track_btn = dialog.addButton("Dont show again", QtWidgets.QMessageBox.ActionRole)
            dialog.setMinimumSize(500, 300)
            dialog.exec_()

            if dialog.clickedButton() == del_track_btn:
                print(lost_tracks)
                dialog.destroy()
                self.playlists[self.current_playlist_name] = [track for track in self.playlists[self.current_playlist_name]
                                                              if track not in lost_tracks]
                self.save_data()
    ...
    def delete_from_playlist(self, path):
        ...
        reply = QtWidgets.QMessageBox.question(
            self,
            self.deleting,
            f"{self.are_you_sure_delete_track} '{os.path.basename(path)}'?",
            QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No
        )
    ...
    def start_loop(self):
            ...
            self.seekbar.setValue(
                int(self.current_length / self.length * 10000)
            )

            if self.is_stopped:
                self.is_played = False
            elif self.is_paused:
                self.current_track_label_2.setText(f"{int(self.current_view_length) // 60:02d}:{int(self.current_view_length) % 60:02d} / {self.view_length // 60:02d}:{self.view_length % 60:02d}")
    ...
    def set_music_to_current_slide_pos(self, value):
        if self.is_stopped:
            ...
            self.current_music_num = value / 10000 * self.length
    ...
    def play_next_track(self):
            ...
            if not self.current_playlist:
                return
            if self.loop:
                if next_track not in self.current_playlist:
                    return
                self.current_music_num = 0
...
""")
        dialog.setWindowTitle(self.web_download_tooltip)
    ...
    def download_youtube_main(self, url):
        ...
        self.web_download_btn.setEnabled(False)
    ...
    def on_download_error(self, err):
        ...
        self.current_track_label.setText(os.path.basename(self.current_track_path))
        if not self.current_track_path:
            self.current_track_label.setText(self.track_not_selected)
        self.web_download_btn.setEnabled(True)
    ...
    def build_ydl_opts(self):
        if not os.path.exists(resource_path("assets/ydlopts.txt")) and build_os == "Darwin":
            return None
        if not os.path.exists(resource_path("assets/ydlopts_win.txt")) and build_os != "Darwin":
            return None
        if build_os == "Darwin":
    ...
    def on_download_finished(self, final_mp3, url):
        ...
        self.current_track_label.setText(self.current_track_path)
        if not self.current_track_path:
            self.current_track_label.setText(self.track_not_selected)
        self.web_download_btn.setEnabled(True)
    ...
    def settings_open(self):
        ...
        self.settings_win.resize(450, 580)
    ...
    def setup_settings_ui(self):
        ...
        self.theme_menu.setGeometry(QtCore.QRect(120, 40, 201, 31))
...
""")
        ...
        self.font_size_menu = QtWidgets.QComboBox(self.settings_win)
        self.font_size_menu.setGeometry(QtCore.QRect(120, 100, 201, 31))
        self.font_size_menu.setStyleSheet("")
        self.font_size_menu.setCurrentText("")
        self.font_size_menu.setObjectName("font_size_select")
        self.font_size_menu.addItems(["4", "6", "9", "10", "12", "14", "16", "18", "20", "22", "24"])
        self.font_size_menu.setCurrentText(str(self.settings.get("font_size")))
        self.font_size_menu.setView(QtWidgets.QListView())
        self.font_size_menu.view().setSpacing(6)
        self.font_size_menu.setCursor(QtGui.QCursor(QtCore.Qt.PointingHandCursor))
        self.font_size_menu.setStyleSheet("""
    QComboBox {
        background-color: #2b2b2b;
        color: #F2F5FA;
...
""")
        self.font_size_menu.currentTextChanged.connect(self.change_font_size)
        
        self.font_size_label = QtWidgets.QLabel(self.settings_win)
        self.font_size_label.setGeometry(QtCore.QRect(130, 80, 181, 15))
        self.font_size_label.setObjectName("font_size_label")
        self.font_size_label.setFont(self.main_font)
        ...
        self.open_downloads_btn.setGeometry(QtCore.QRect(120, 140, 201, 32))
        ...
        self.open_stdout_btn.setGeometry(QtCore.QRect(120, 180, 201, 32))
        ...
        self.open_keybinds_btn.setGeometry(QtCore.QRect(120, 220, 201, 32))
        ...
        self.tutorial_btn.setGeometry(QtCore.QRect(120, 260, 201, 32))
        ...
        self.language_label.setGeometry(QtCore.QRect(10, 300, 421, 20))
        ...
        self.language_label_icon.setGeometry(QtCore.QRect(90, 332, 51, 31))
        ...
        self.lang_menu.setGeometry(QtCore.QRect(120, 330, 201, 31))
...
""")
        self.lang_menu.addItems(["English", "Русский", "Deutsch", "ქართული", "español", "Українська", "Қазақ", "Polski", "Français", "日本語", "中國人", "Italiano", "Azərbaycan dili", "беларуская", "Türkçe", "عربي", "Română", "հայ", "Português", "বাংলা"])
        ...
        self.dev_label.setGeometry(QtCore.QRect(10, 370, 421, 20))
        ...
        self.mady_by_label.setGeometry(QtCore.QRect(10, 400, 421, 20))
        ...
        self.github_btn.setGeometry(QtCore.QRect(120, 430, 201, 32))
        ...
        self.tiktok_btn.setGeometry(QtCore.QRect(120, 470, 201, 32))
        ...
        self.version_label.setGeometry(QtCore.QRect(10, 550, 421, 20))
        ...
        self.icon_creators_btn.setGeometry(QtCore.QRect(120, 510, 201, 32))
        ...
        self.tutorial_btn.clicked.connect(self.open_color_config)
    ...
    def setup_settings_text(self):
        ...
        self.tutorial_btn.setText("Colors Config (EXP!)")
        ...
        self.version_label.setText(f"Version: {script_version}")
    ...
    def change_font_size(self, size):
        ...
        self.save_settings()
        self.main_font.setPointSize(size)
        self.apply_font()
        self.settings_open()
    ...
    def open_color_config(self):
        """COLOR CONFIG"""
        bg = self.colors.get("bg_color")
        self.cc_win = QtWidgets.QDialog(self.settings_win)
        self.cc_win.setWindowTitle("Colors config (EXPERIMENTAL!)")
        self.cc_win.setObjectName("CCWIN")
        self.cc_win.resize(320, 190)
        self.cc_win.setLayoutDirection(QtCore.Qt.LeftToRight)
        self.cc_win.setModal(True)
        self.cc_win.setWindowFlags(
            QtCore.Qt.Window |
            QtCore.Qt.CustomizeWindowHint |
            QtCore.Qt.WindowCloseButtonHint |
            QtCore.Qt.WindowMinimizeButtonHint
        )
        self.cc_win.setStyleSheet("QPushButton{\n"
                                "    border-radius: 15px;\n"
                                "    background-color: #32323c;\n"
                                "    color: white;\n"
                                "    border-color: "
    ...
    def cc_win_ui(self):
        ...
        self.warn_label = QtWidgets.QLabel(self.cc_win)
        self.warn_label.setGeometry(QtCore.QRect(10, 0, 300, 60))
        self.warn_label.setFont(self.main_font)
        self.warn_label.setText("WARNING: This feature is experimental.\nIt might crash your app. Report bugs on github.")
        self.warn_label.setStyleSheet("color: white;")
        self.warn_label.setAlignment(QtCore.Qt.AlignCenter)

        self.islands_color_change_btn = QtWidgets.QPushButton(self.cc_win)
        self.islands_color_change_btn.setText("Islands Color")
        self.islands_color_change_btn.setStyleSheet("""
                                border-color: #7476a0;
                                color: white;
                                border: 2px solid #7476a0""")
        self.islands_color_change_btn.setGeometry(QtCore.QRect(60, 70, 200, 30))
        self.islands_color_change_btn.setCursor(QtGui.QCursor(QtCore.Qt.PointingHandCursor))
        self.islands_color_change_btn.setFont(self.main_font)
        self.islands_color_change_btn.clicked.connect(self.change_islands_color)

        self.bg_color_change_btn = QtWidgets.QPushButton(self.cc_win)
        self.bg_color_change_btn.setText("Background Color")
        self.bg_color_change_btn.setStyleSheet("""
                                border-color: #7476a0;
                                color: white;
                                border: 2px solid #7476a0""")
        self.bg_color_change_btn.setGeometry(QtCore.QRect(60, 105, 200, 30))
        self.bg_color_change_btn.setFont(self.main_font)
        self.bg_color_change_btn.setCursor(QtGui.QCursor(QtCore.Qt.PointingHandCursor))
        self.bg_color_change_btn.clicked.connect(self.change_bg_color)

        self.reset_to_defalut_btn = QtWidgets.QPushButton(self.cc_win)
        self.reset_to_defalut_btn.setText("Reset to default")
        self.reset_to_defalut_btn.setGeometry(QtCore.QRect(60, 140, 200, 30))
        self.reset_to_defalut_btn.setFont(self.main_font)
        self.reset_to_defalut_btn.setStyleSheet("""
                                border-color: #7476a0;
                                color: white;
                                border: 2px solid #7476a0""")
        self.reset_to_defalut_btn.setCursor(QtGui.QCursor(QtCore.Qt.PointingHandCursor))
        self.reset_to_defalut_btn.clicked.connect(self.reset_to_default)

    def change_islands_color(self):
        dialog = QtWidgets.QColorDialog(self.cc_win)
        dialog.setCurrentColor(QtGui.QColor(self.colors.get("islands_color")))
        pos = self.islands_color_change_btn.mapToGlobal(QtCore.QPoint(0, self.islands_color_change_btn.height()))
        dialog.move(pos)

        if dialog.exec_():
            dialog.destroy()
            islands_color = dialog.selectedColor().name()
            self.colors["islands_color"] = islands_color
            self.sidebar.setStyleSheet(f"""background-color: {islands_color};
                                            border-radius: 10px;
                                            """)
            self.leftbar.setStyleSheet(f"""background-color: {islands_color};
                                            border-radius: 10px;""")
            self.save_colors()

    def change_bg_color(self):
        dialog = QtWidgets.QColorDialog(self.cc_win)
        dialog.setCurrentColor(QtGui.QColor(self.colors.get("bg_color")))
        pos = self.bg_color_change_btn.mapToGlobal(QtCore.QPoint(0, self.bg_color_change_btn.height()))
        dialog.move(pos)

        if dialog.exec_():
            dialog.destroy()
            bg_color = dialog.selectedColor().name()
    ...
    def reset_to_default(self):
        bg_color = "#1d1f28"
        islands_color = "#292934"
        self.colors["bg_color"] = "#1d1f28"
        self.colors["islands_color"] = "#292934"
        self.save_colors()
